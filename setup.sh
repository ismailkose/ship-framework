...
# SKILL.md frontmatter hooks only register once that skill is invoked. The
# always-on hooks (refgate, sessionstart) must live in .claude/settings.json.

SETTINGS_FILE="$TARGET_DIR/.claude/settings.json"
if [ -f "$SETTINGS_FILE" ]; then
  # settings.json exists — add any missing Ship hooks alongside existing ones.
  # Idempotent: never replaces or duplicates hooks the user (or a previous run) set.
  ADDED=$(python3 -c "
import json
path = '$SETTINGS_FILE'
with open(path) as f:
...
" 2>/dev/null) || ADDED="error"
  case "$ADDED" in
    error) echo -e "${YELLOW}⚠${RESET} Could not merge hooks into existing settings.json — add manually" ;;
    0)     echo -e "${DIM}  Hooks already registered in settings.json${RESET}" ;;
    *)     echo -e "${GREEN}✓${RESET} Registered hooks in .claude/settings.json (refgate, sessionstart)" ;;
  esac
else
